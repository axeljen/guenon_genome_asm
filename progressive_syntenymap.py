...
(it could use substantial cleaning...)
...
def write_chromdicts(chromdicts, outfile):
	...
	with open(outfile, 'w') as out:
		# write the header
		out.write('chrom\tcumpos\tlength\tgenome\tstrand\n')
		...
		for i,chromdict in enumerate(chromdicts):
			...
			for chrom in chromdict.keys():
				...
				out.write('{}\t{}\t{}\t{}\t{}\n'.format(chrom, chromdict[chrom]['cumpos'], chromdict[chrom]['length'], str(i + 1), chromdict[chrom]['strand']))
...
if __name__ == '__main__':
	...
	#for i,aln in enumerate(['map_Cercopithecus_diana_hap1_asm_ref.paf','map_Allochrocebus_lhoesti_hap1_asm_ref.paf']):
		...
		aln_dict = filterPAF(aln_dict, minlen=args.minlen, primary=True, minmapq=args.minmapq, minpid=args.minpid, only_ref_chroms=True, only_query_chroms=True, ref_chroms=refgenome.keys(), query_chroms=genomes[i].keys())
		...
		if not args.no_flip:
			for chrom in genomes[i].keys():
				...
				genomes[i][chrom]['strand'] = strand
				#print(genomes[i][chrom])
	...
	# first add plus strand to all refgenome chromosomes
	for chrom in refgenome.keys():
		refgenome[chrom]['strand'] = '+'
